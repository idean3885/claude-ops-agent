...
### Added
* 제출 시점에 코드 변경분을 보는 판정 주체를 둔다 (#511)

### Changed
* PR 을 초안으로 열고 리뷰 요청을 준비 완료 전환에 붙인다 (#505)
* 알림 발송처가 선언되지 않은 레포는 준비 완료 전환으로 리뷰 요청 단계를 끝낸다 (#505)

### Removed
* 레슨런 후보 적립 훅과 후보 파일 기제를 제거하고 판정 입력을 세션 트랜스크립트 단일로 둔다 (#516)

## [9.4.0] - 2026-09-20

### Added
* 기획 전문가에 Figma 조회 절차(MCP 우선, 한도 시 Chrome DevTools MCP) 추가 (#503)
* 도구 구성은 공식 문서를 기본값으로 삼고 벗어날 때만 ADR 을 쓴다 (#507)
* 한국어 문서 규칙 색인을 신설하고 미러 대상에 넣는다 (#507)

### Changed
* 전역 지침 조각에서 훅·스킬·레포 문서가 이미 하는 절을 걷는다 (#507)

## [9.3.0] - 2026-09-16

### Added
* 아키텍트 전문가에 도메인 설계 판정과 검토 동행을 더한다 (#467)
* 착수 전 필수 확인을 두고 PR 가이드가 리뷰 도구와 전문가를 가리킨다 (#471)

### Changed
* 작업 모드(절감·풀)를 도입하고 모델 의존과 비용 제약을 나눈다 (#471)

### Fixed
* 동기 스크립트가 받은 것이 없는 실행을 구분해 알린다 (#488)

## [9.2.0] - 2026-09-16

### Added
* 보안 전문가 추가. infra-lab·cluster-check 이 외부에 맡기던 구성 보안 자리를 내부 전문가로 잇는다 (#479)
* 전문가 추가 기준을 스킬에 명시. 사례가 서로 다른 자리에서 두 번 나온 뒤에 만든다 (#479)

## [9.1.0] - 2026-09-16

### Added
* 아키텍트·인프라 전문가 추가. infra-lab 이 이름만 부르던 자리가 정의 파일로 이어진다 (#479)

## [9.0.0] - 2026-09-16

### Added
* 저자 톤 T23 「정착한 외래어를 순화어로 바꾸지 않는다」 추가. 금지 표현 「도식」은 그 사례로 등록 (#477)
* AI 티 F-8 한자 접두 조어 신설. 未·非·無·不 + 명사로 만든 사전 밖 낱말을 판정한다 (#480)
* 기획 전문가와 expert 스킬 추가. 전문가가 판정 기준으로 의견을 내고 개발자가 반박한다 (#479)

### Changed
* **BREAKING** advisor·cross-verify 를 전문가 토론 체계로 교체 (#479)

## [8.18.0] - 2026-09-15

### Added
* 소제목을 물음으로 다는 것을 금지하는 규칙을 tone.md T18 에 등재. 레포 문서의 의문형 소제목 13곳을 명사구로 교체 (#469)

### Changed
* readability.md H4 의 질문형 헤딩 어미 규정을 걷고 tone.md T18 을 가리키게 함 (#469)
* 머지 승인 이후 게이트 여는 명령을 어시스턴트가 먼저 제시하도록 플로우를 잇는다 (#472)

### Fixed
* 한시 권한 가드가 따옴표 안의 구분자를 명령 경계로 보지 않는다 (#473)
