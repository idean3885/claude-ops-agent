...
- **GATE 3 (한시 권한 개방)**: 되돌리기 어려운 행위(PR 머지·릴리즈·force push·리소스 삭제 등)는 `scripts/pre-tool-use.mjs` 가 막는다. 어시스턴트가 권한을 스스로 열지 않는다. 판정 주체는 항상 사람이다.
  ...
  - **게이트가 필요하다는 것을 작업 중에 알았으면 그 시점에 요청한다.** 멈추기 어려우면 게이트 없이 되는 일을 먼저 끝내고 마지막에 요청한다. 미루는 것은 요청이지 행위가 아니며, 대상 행위는 창이 열린 뒤에만 실행한다.
...
머지는 사용자가 웹에서 직접 수행하는 것이 기본이다 (세이프티가드). **사용자가 머지를 승인했으면 되묻지 말고 GATE 3 으로 들어가 여는 명령을 제시한다.** 사용자가 게이트를 먼저 꺼내게 두지 않는다. 승인은 2단계를 통과시킬 뿐이므로, 명령을 제시하는 것과 권한이 열리는 것은 여전히 별개다.
