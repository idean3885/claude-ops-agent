...
이슈 플로우 전체를 오케스트레이션하는 워크플로우

## 역할

자연어 수정 요청을 받아 이슈 탐색부터 PR 머지까지 전체 플로우를 안내한다.
...
- "중단", "취소", "stop", "cancel" 요청 시 즉시 플로우를 중단하고 현재 상태를 보고
- GATE에서 수정 요청이 오면 해당 Phase를 재수행

## Provider 연동

각 Phase에서 이슈 트래커 조작이 필요할 때:
1. 현재 세션의 provider 확인 (session-start 훅에서 감지됨)
2. provider 정의 파일에서 해당 동작의 API/명령 참조
3. provider가 없으면 기본 내장 provider (github) 사용
...
2. provider의 이슈 검색 API로 기존 이슈 탐색
3. 매칭되는 이슈가 있으면 재사용 제안, 없으면 신규 생성
4. 신규 생성 시 `/issue` 스킬의 워크플로우를 따른다
...
1. 승인된 플랜을 기반으로 구현 수행
2. 구현 중 플랜 변경 필요 시 사용자에게 보고
...
> 반드시 사용자의 명시적 응답을 기다린다.
> 사용자가 머지를 원하지 않으면 PR 링크만 전달하고 플로우를 종료한다.

### Phase 7: PR 머지 + 브랜치 정리

1. provider의 PR 머지 API/명령 실행
2. 작업 환경 정리:
...
- 개별 스킬(`/issue` 등)의 단독 사용에 영향을 주지 않는다
