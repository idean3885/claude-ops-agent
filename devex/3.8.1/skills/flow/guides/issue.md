...
6. **대외비 검증** (퍼블릭 리모트 대상):
   - 이슈 제목·본문에 사내 서비스명, 회사명, 내부 도메인, 사내 URL이 포함되어 있지 않은지 확인
   - 위반 발견 시 이슈 생성을 차단하고 사용자에게 보고
7. provider의 이슈 생성 API/명령 실행 (provider에 정의된 인증·endpoint·body 그대로 사용)
8. **Dailylog 장기 계획 추가** (필수):
   - 생성한 이슈를 금일 데일리로그의 **장기 계획** 섹션에 추가
   - 하위 이슈(`parentPostId` 있음)인 경우: 상위 이슈 아래에 들여쓰기로 추가 (상위가 장기 계획에 없으면 상위 먼저 추가)
   - 독립 이슈인 경우: 플랫하게 추가, 소제목(####) 카테고리 분류 적용
   - provider의 Extensions > Dailylog Integration 규칙을 따른다
...
6. **코드 작업 여부 확인**: "코드 작업이 필요한 이슈입니까?"
   ...
   - **No**: 브랜치 생성 스킵
7. **Dailylog 일일 계획 추가** (필수):
   - 장기 계획에 해당 이슈가 있으면 → 장기 계획에서 제거하고 **일일 계획**으로 이동
   - 장기 계획에 없으면 → 일일 계획에 새 항목 추가
   - **즉시 시작 (생성+시작 연속 요청)**: 장기 계획 거치지 않고 바로 일일 계획에 추가
   - 상위-하위 계층 구조 및 소제목 카테고리 분류 유지
   - provider의 Extensions > Dailylog Integration 규칙을 따른다
...
8. workflow.json의 currentIssue 제거
9. **Dailylog 완료 처리**: toolkit의 `dooray-dailylog` 스킬 규칙을 따른다. 일일 계획에서 `[x]` 체크만 수행. 완료 작업/주간 보고 이관은 다음날 데일리로그 생성 시점에 처리된다.
10. 기타 Extensions 실행 (provider에 정의되어 있으면)
...
- **퍼블릭 리모트 대상 이슈의 제목·본문에 대외비(사내 서비스명, 회사명, 내부 URL 등)를 포함하지 않는다**
