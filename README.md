...
개발 워크플로우에서 반복되는 이슈 관리·문서 작성·검증을 자동화하려고 직접 만들어 쓰는 개인용 Claude Code 플러그인입니다.

> 자주 쓰는 워크플로우와 작성 규칙을 하나로 묶고,
> 세션 시작·파일 편집·응답 시점에 자동으로 적용되도록 환경에 넣어 두었습니다.

## 무엇을 하나

AI 에게 코드와 문서를 맡기는 비중이 늘면서, 같은 지시를 매번 반복하고 결과를 교정하는 일이 늘었습니다. 이 반복을 줄이려고 자주 쓰는 세 가지 흐름을 플러그인 하나로 묶었습니다.

- **이슈·멀티레포 플로우**: 자연어 수정 요청을 이슈 → spec → 구현 → commit → PR 로 이어 주고, 여러 서비스 레포에 걸친 변경은 org-flow 로 통일 브랜치·워크트리·계정을 한 흐름에 맞춥니다.
- **콘텐츠 작성·검증**: 한국어 문서를 작성하고 AI 티·가독성·톤을 점검한 뒤 발행합니다.
- **규칙 자동 적용**: 위 두 흐름에 쓰이는 규칙을 세션·편집·응답 시점에 자동으로 겁니다.

규칙을 그때그때 지시하는 대신 환경에 넣어 두어, 매 세션 같은 상태에서 작업을 시작합니다. 이렇게 만든 배경은 [docs/design-philosophy.md](docs/design-philosophy.md) 에 따로 적었습니다.
...
| 축 | 구성요소 | 역할 |
|----|----------|------|
| **이슈·멀티레포 플로우** | `flow`, `org-flow`, `setup`, provider 시스템 | 단일 레포는 flow, 여러 레포에 걸친 변경은 org-flow 로 조율 |
| **콘텐츠 작성·검증** | `content-write`, `content-verify`, `content-publish`, `cross-verify`, 스타일 SSOT | 작성 → 검증 → 발행, 그리고 의도적 멈춤(교차 검증) |
| **규칙 자동 적용** | SessionStart, hook(표현 가드·content-verify·대외비), 스킬 트리거 | 작성 원칙·워크플로우 규칙을 환경에 고정 |
...
### 멀티레포 오케스트레이션 (org-flow)

MSA 에서 서비스가 레포 단위로 나뉘면 하나의 변경이 여러 레포에 걸칩니다. `org-flow` 는 매니페스트에 정의한 레포 세트에 대해 통일 브랜치명(`feature/{ticket}`)을 관련 레포에만 만들고, 레포별 provider·Git Identity 를 자동 분기하며, 레포별 워크트리를 격리 생성합니다. 여러 레포에 걸친 변경에서 브랜치·계정·순서가 어긋나지 않게 한 흐름으로 맞춥니다. 최초 호출 시 org 셋업 마법사가 매니페스트를 생성합니다.
...
작성 원칙과 워크플로우 규칙을 사람의 기억이 아니라 환경에 고정합니다. 세 시점에 규칙이 걸립니다. 시점마다 막는 실수가 다릅니다. 설계 배경은 [docs/design-philosophy.md](docs/design-philosophy.md) 를 참조하세요.
...
├── docs/
│   ├── design-philosophy.md         # 설계 배경 (왜 규칙을 환경에 넣는가)
