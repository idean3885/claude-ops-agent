...
> 작성 원칙과 워크플로우 규칙을 하네스(SessionStart·hook·스킬)에 주입해 매번 같은 출발선에서 동작시킨다.

## 배경

AI 에게 코드를 맡기면서 개발자의 역할이 "코드 작성"에서 "의사결정과 검증"으로 옮겨갔다.

문제는 AI 가 매번 같게 동작하지 않는다는 점이다. 톤이 흔들리고, 규칙을 잊고, 묻지 않은 수식어를 붙인다. 그래서 규칙을 기억에 의존하지 않고 하네스에 심어, 매 세션·매 편집·매 응답에 자동으로 걸리게 만든다. 이것이 이 플러그인이 AI 를 대하는 방식이다.

설계 의도와 주입 메커니즘의 상세는 [docs/design-philosophy.md](docs/design-philosophy.md) 에 정리했다.
...
세 축으로 묶인다. 앞의 두 축이 "무엇을 하는가"이고, 세 번째 축이 "어떻게 매번 같게 동작시키는가"이다.
...
보조로 ticket 단위 토큰·비용 추적([Usage Tracking](#usage-tracking))을 제공한다.
...
`flow` 스킬 하나가 전체 플로우를 오케스트레이션한다. 자연어로 수정을 요청하면 git 상태를 감지해 현재 단계에 맞는 작업을 실행하고, 3개의 확인 게이트(플랜 승인, 커밋 승인, 머지 승인)에서 사용자 승인을 받은 후 진행한다.
...
issue / spec / commit / pr 단계의 상세 가이드는 `flow` 스킬 내부의 `guides/` 로 통합되어 있다. 단계 진입 시에만 로딩되어 컨텍스트를 아낀다.

### Provider 시스템

이슈 트래커별 동작을 provider 로 추상화한다. SessionStart 훅에서 git remote host 기반으로 자동 감지된다.
...
Provider별 Git Identity(user.name, user.email)를 정의해, 커밋/푸시 시 올바른 계정으로 자동 설정한다.

- SessionStart 훅에서 `gh auth status` 크리덴셜과 provider identity 를 매칭
- 커밋 전 `git config user.name/email` 을 provider 기준으로 자동 검증·수정
- 글로벌 git config 에 의존하지 않아 계정 오류를 원천 차단

---

## 2. 콘텐츠 작성·검증

블로그·위키·이슈·PoC 등 한국어 문서를 작성하고 검증하는 흐름이다. 작성 엔진이 초안을 만들고, 검증이 AI 티·가독성·톤을 점검하고, 발행이 Jekyll 로 변환한다.
...
자동화가 아니라, **개발자가 의도적으로 멈추고 확인하는 행위를 구조화한 것**이다. 도구가 측정할 수 없는 의미적 판단에 집중한다. `config/cross-verify/profiles/` 의 프로젝트별 프로필을 자동 매칭하며, 프로필이 없으면 범용 4축 검증을 실행한다.

### 스타일 룰 (Style Rules) — 작성 SSOT

블로그·위키·이슈·PoC·데일리로그·동료리뷰·성과평가 등 **모든 한국어 문서**에 적용되는 작성 단일 출처다. base(공통) + extensions(유형별) 구조로, 작성 빈도와 함께 점진 보강한다.
...
**AI 티 분류**는 [`epoko77-ai/im-not-ai`](https://github.com/epoko77-ai/im-not-ai) (MIT) 의 10대 분류 골격(A~J)과 심각도(S1/S2/S3) 체계를 차용했다. 처방·예시·hook 매핑은 한국어 기술 블로그 맥락으로 자체 작성한 파생물이다.

SessionStart hook 이 `config/style-rules/` 를 `~/.claude/devex/style-rules/` 로 미러링한다. toolkit 등 외부 소비자는 이 경로를 참조한다. 사용자 로컬 추가 룰(`*.local.md`)은 미러 시 덮어쓰지 않는다.

---

## 3. 사상 주입 하네스

작성 원칙과 워크플로우 규칙을 사람의 기억이 아니라 환경에 고정한다. 세 시점에 규칙이 걸린다. 시점마다 막는 실수가 다르다. 설계 의도는 [docs/design-philosophy.md](docs/design-philosophy.md) 참조.
...
금지 표현(과장형 형용사·보고서체·근거 없는 단언·번역투 등)을 응답 출력 직전에 막거나 자동으로 고쳐 쓰지 않는다. UserPromptSubmit 가 룰을 사전 주입하고, Stop 이 직전 응답 위반을 사후 통지한다. 따라서 출력 직전 패턴 자가 대조는 어시스턴트의 의무이며, hook 은 이를 돕는 사전 가이드·사후 통지 역할이다.

룰은 `base/ai-tells.md` 의 카테고리 ID(`taxonomyId`)와 1:1 매핑되어, 패턴이 왜 존재하는지 역추적된다.
...
룰 추가는 JSON 에 `{pattern, replacement, reason}` 객체 하나만 추가하면 즉시 반영된다(Python 정규식). 신규 패턴은 먼저 `base/ai-tells.md` 분류 체계에 카테고리 ID 부여 후 S1 으로 판정될 때 등록한다.

### content-verify 자동 점검 (opt-in)

문서 편집(Edit/Write) 직후 content-verify 관점(AI 티·가독성·톤·구두점) 자가 점검을 유도하는 PostToolUse hook 이다. 프로젝트 루트에 마커 파일(`.devex/content-verify.json`)이 있을 때만 작동한다.
...
ticket 단위로 토큰·비용을 추적하는 스킬 묶음이다. worktree-per-task 환경에서 정확히 분리된다.
...
worktree-per-task 환경에서 ticket 단위 분리가 동작하는 원리는 [docs/usage-cwd-aggregation.md](docs/usage-cwd-aggregation.md) 참조.

---

## 설치

Claude Code 플러그인 마켓플레이스에서 설치한다.
...
캐시 디렉토리에서 직접 수정 → 커밋 → 푸시. 다음 세션 시작 시 버전 동기화가 자동 수행된다.
