...
| 전문가 | 무엇에 의견을 갖는가 |
|--------|---------------------|
| 기획 | 문제 정의·범위·성공 지표가 검증 가능한가 |
| 아키텍트 | 경계와 책임 분리, 기각한 대안이 근거인가 취향인가 |
| 인프라 | 용량 근거·첫 병목·비용·철거 |
| 보안 | 접근 범위·자격의 성질·노출 경계·폐기 경로 |
...
| 한계가 있는 자리 | 지금 상태 |
|------------------|-----------|
| 기준 모델 | 발화 분량·검증·위임 규칙은 Claude Opus 5 로 쓰면서 다듬었습니다. 규범과 비용 제약은 모델이 바뀌어도 유지되고, 모델 경향을 누르는 항목만 재판정 대상입니다 ([구분](docs/effort-policy.md#모델-의존과-비용-제약을-나눈다)) |
...
| 전문가 범위 | 분야를 하나씩 채우는 중입니다. 없는 분야를 물으면 그 사실을 말하고 멈춥니다 ([목록](skills/expert/SKILL.md)) |
